 ...
 * @since unknown
 */
class Chatter_Recent_Comments extends WP_Widget {
	function __construct() {
		parent::__construct( false, __( 'Chatter Recent Comments', 'Chatter' ), array( 'description' => __( 'Recent comments with avatars.', 'Chatter' ) ) );
	...
	function flush_widget_cache() {
		wp_cache_delete( 'Chatter_recent_comments', 'widget' );
	}
	...
	function form( $instance ) {
		...
		$sizes = array(
			'32' => 'Yes &mdash; 32x32',
			'-1' => __( 'No avatar', 'Chatter' ),
		);
?>
	<p>
		<label for="<?php echo $title_id ?>"><?php _e( 'Title:', 'Chatter' ); ?>
			<input type="text" class="widefat" id="<?php echo $title_id ?>" name="<?php echo $title_name ?>"
				value="<?php echo $title; ?>" />
		</label>
	</p>
	<p>
		<label for="<?php echo $num_to_show_id ?>"><?php _e( 'Number of comments to show:', 'Chatter' ); ?>
			<input type="text" class="widefat" id="<?php echo $num_to_show_id ?>" name="<?php echo $num_to_show_name ?>"
				value="<?php echo $num_to_show; ?>" />
		</label>
	</p>
	<p>
		<label for="<?php echo $avatar_size_id ?>"><?php _e( 'Avatars:', 'Chatter' ); ?>
...
<?php
	...
	function widget( $args, $instance ) {
		...
		$title = (isset( $instance['title'] ) && $instance['title'])? $instance['title'] : __( 'Recent comments', 'Chatter' );
...
?>
		<table class="Chatter-recent-comments" cellspacing="0" cellpadding="0" border="0">
	...
	static function comment_url_maybe_local( $comment ) {
		...
		// Works by detecting if the comment's post is visible on the page... may break if Chatter decides to do clever stuff with comments when paginated
		$comment_url = get_comment_link( $comment );

		if ( '1' === get_option( 'Chatter_hide_threads' ) )
			return $comment_url;
	...
	function recent_comments( $num_to_show ) {
		global $wpdb;
		$cache = wp_cache_get( 'Chatter_recent_comments', 'widget' );
		if ( !is_array( $cache ) ) {
			$cache = array();
		}
		...
		wp_cache_add( 'Chatter_recent_comments', $cache, 'widget' );
		return $comments;
	}
	...
	static function single_comment_html($comment, $avatar_size ) {
		...
		if ( !$comment->comment_author ) $comment->comment_author = __( 'Anonymous', 'Chatter' );
		...
		$comment_url = Chatter_Recent_Comments::comment_url_maybe_local( $comment );

		$row .= sprintf( '<td class="text">'.__( "%s on <a href='%s' class='tooltip' title='%s'>%s</a>" , 'Chatter' ) . '</td></tr>', $author_html, esc_url( $comment_url ), esc_attr($comment_content), $post_title );
		return $row;
	}
}

register_widget( 'Chatter_Recent_Comments' );
