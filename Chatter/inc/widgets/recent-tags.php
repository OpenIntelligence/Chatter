 ...
 * @since unknown
 */
class Chatter_Recent_Tags extends WP_Widget {
	function __construct() {
		parent::__construct( false, __( 'Chatter Recent Tags', 'Chatter' ), array( 'description' => __( 'The tags from the latest posts.', 'Chatter' )));
...
?>
	...
	<p>
		<label for="<?php echo $num_to_show_id ?>"><?php _e( 'Number of tags to show:', 'Chatter' ); ?>
...
<?php
	...
	function widget( $args, $instance ) {
		...
		$title = (isset( $instance['title'] ) && $instance['title'])? $instance['title'] : __( 'Recent tags', 'Chatter' );
	...
	function recent_tags( $num_to_show ) {
		$cache = wp_cache_get( 'Chatter_recent_tags', 'widget' );
		if ( !is_array( $cache ) ) {
			$cache = array();
		}
		...
		wp_cache_add( 'Chatter_recent_tags', $cache, 'widget' );
		return $recent_tags;
	}

}

register_widget( 'Chatter_Recent_Tags' );