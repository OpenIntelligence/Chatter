...
class ChatterAjax_Read {
	static function dispatch() {
		$action = isset( $_REQUEST['action'] ) ? $_REQUEST['action'] : '';

		do_action( "Chatter_ajax", $action );
		if ( is_callable( array( 'ChatterAjax_Read', $action ) ) ) {
			status_header( 200 );
			call_user_func( array( 'ChatterAjax_Read', $action ) );
		} else {
			die( '-1' );
		}
		exit;
		...
		if ($number_of_new_comments > 0) {
			foreach ($comments as $comment) {
				...
				if ( in_array( $comment->comment_post_ID, $visible_posts ) ) {
					ob_start();
					Chatter_comments($comment, array( 'max_depth' => $max_depth, 'before' => ' | ' ), $depth );
				...
				$comment_widget_html = '';
				if ( $lc_widget )
					$comment_widget_html = Chatter_Recent_Comments::single_comment_html( $comment, $avatar_size );
	...
	static function new_comment() {
		...
		if ( is_user_logged_in() ) {
			...
			if ( get_option( 'comment_registration' ) ) {
			    die( '<p>'.__( 'Error: you must be logged in to post a comment.', 'Chatter' ).'</p>' );
			}
		...
		if ( get_option( 'require_name_email' ) && !$user->ID )
			if ( strlen( $comment_author_email ) < 6 || '' == $comment_author ) {
				die( '<p>'.__( 'Error: please fill the required fields (name, email).', 'Chatter' ).'</p>' );
			} elseif ( !is_email( $comment_author_email ) ) {
			    die( '<p>'.__( 'Error: please enter a valid email address.', 'Chatter' ).'</p>' );
			}

		if ( '' == $comment_content )
		    die( '<p>'.__( 'Error: Please type a comment.', 'Chatter' ).'</p>' );
		...
		else echo __("Error: Unknown error occurred. Comment not posted.", 'Chatter' );
	}
}
