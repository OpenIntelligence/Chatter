		...
		<ul id="postlist">
			<li id="prologue-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<h4>
					...
					<span class="meta">
						<?php echo Chatter_date_time_with_microformat(); ?>
						<?php
							$metadata = wp_get_attachment_metadata();
							if ( wp_attachment_is_image() ) {
								echo " | "; // Pipe separator.
								printf( __( 'Full size is %s pixels', 'Chatter' ),
								sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
									wp_get_attachment_url(),
									esc_attr( __( 'Link to full-size image', 'Chatter' ) ),
										$metadata['width'],
										$metadata['height']
									)
								);
						...
						<span class="actions">
							...
							if ( current_user_can( 'edit_post', get_the_ID() ) ) : ?> | <a href="<?php echo ( get_edit_post_link( get_the_ID() ) ); ?>" rel="<?php the_ID(); ?>" title="<?php esc_attr_e( 'Edit', 'Chatter' ); ?>"><?php _e( 'Edit', 'Chatter' ); ?></a>
							<?php endif; ?>
				...
				<?php else : ?>
					<p><?php _e( 'View file:', 'Chatter' ); ?> <a href="<?php echo wp_get_attachment_url(); ?>" title="<?php echo esc_attr( get_the_title() ); ?>" rel="attachment"><?php echo basename( get_permalink() ); ?></a></p>
					<?php the_content( __( '(More ...)' , 'Chatter' ) ); ?>
					<?php wp_link_pages(); ?>
				<?php endif; ?>
				</div><!-- /.postcontent -->
				<?php
