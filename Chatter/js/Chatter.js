...
(function( $ ){
	...
	Chatter.initialize = function() {
		if ( ChatterStoredVersion < ChatterCurrentVersion ) {
			// Chatter upgrades on admin_init.  If it needs it, send a noop admin-ajax call to trigger.
			$.ajax( ajaxUrl, {
				data: {
					action: 'Chatter.noop'
				},
		...
		// ------------------------------------------------
		Chatter.keyboard();
		Chatter.comment.highlight();
		Chatter.utility.localizeMicroformatDates();

		// Initialize mention autocompletes if mention data exists.
		if ( mentionData )
			Chatter.autocomplete.mentions( $( '#posttext, #comment' ) );

		// Activate Tag Suggestions for logged users on front page
		if (isFrontPage && prologueTagsuggest && isUserLoggedIn)
			Chatter.autocomplete.tags( $( 'input[name="tags"]' ) );

		// Turn on automattic updating
		if (prologuePostsUpdates)
			Chatter.utility.toggleUpdates('unewposts');
		if (prologueCommentsUpdates)
			Chatter.utility.toggleUpdates('unewcomments');

		// Activate autgrow on textareas
		Chatter.utility.autoGrowTextarea($('#posttext, #comment'));

		// Activate tooltips on recent-comments widget
		Chatter.utility.tooltip($("table.Chatter-recent-comments a.tooltip"));

		// Hide error messages on load
		$('#posttext_error, #commenttext_error').hide();

		// Bind actions to comments and posts
		$( 'body .post, body .page' ).each( function() { Chatter.utility.bindActions( this, 'post' ); } );
		$( 'body .comment' ).each( function() { Chatter.utility.bindActions( this, 'comment' ); } );
		...
		if	($("table.Chatter-recent-comments").length != 0)
			lcwidget = true;
		...
		$(window).bind('beforeunload', function(e) {
			if ( $( '#posttext' ).val() || $( '#comment' ).val() ) {
				...
				if (e) { // For IE and Firefox
					e.returnValue = Chattertxt.unsaved_changes;
				}
				return Chattertxt.unsaved_changes;   // For Safari
			}
		});
		...
		$("#new_post").submit(function(trigger) {
			...
			Chatter.posts.submit(trigger);
			trigger.preventDefault();
		});

		// Catch new comment submit
		$("#commentform").bind( 'submit', function(trigger) {
			Chatter.comment.submit(trigger);
			trigger.preventDefault();
			$(this).parents("li").removeClass('replying');
			$(this).parents('#respond').prev("li").removeClass('replying');
		});

		// Let's do a check for logged-in (and maybe being offline) status, since Chatter thinks current user is logged in.
		if ( isUserLoggedIn ) {
				...
				$.ajax({
					...
					success: function( response ) {
						if ( 'logged_in' != response ) {
							...
							Chatter.utility.notification( Chattertxt.oops_not_logged_in + ' <a href="' + login_url + '">' + Chattertxt.please_log_in + '</a>.', true );
					...
					error: function() {
						// Chatter, we have a problem.
						Chatter.utility.notification( Chattertxt.whoops_maybe_offline );
						event.preventDefault();
						return false;
					}
				});
		...
		$(window).scroll(function() { Chatter.utility.removeYellow(); });
	...
	Chatter.comment = {
				...
				if (newComments != null) {
					$.each(newComments.comments, function(i,comment) {
						...
						if (comment.widgetHtml) {
							Chatter.comment.insert.widget(comment.widgetHtml);
						}
						if (comment.html != '') {
							var thisParentId = 'prologue-'+comment.postID;
							Chatter.comment.insert.inline(thisParentId, comment.commentParent, comment.html, showNotification);
						}
					});
					if (showNotification) {
						Chatter.utility.notification(Chattertxt.n_new_comments.replace('%d', newComments.numberofnewcomments));
					}
				}
			});
			Chatter.utility.toggleUpdates('unewcomments');
		...
		insert: {
			...
			inline: function( postParent, comment_parent, commentHtml, showNotification ) {
				...
				newCommentsLi.slideDown( 200, function() {
					...
					newComment.children('li.newcomment').each(function() {
						if (Chatter.utility.isElementVisible(this) && !showNotification) {
							$(this).animate({backgroundColor:'transparent'}, {duration: 1000}, function(){
								$(this).removeClass('newcomment');
							});
						}
						Chatter.utility.bindActions(this, 'comment');
					});
					Chatter.utility.localizeMicroformatDates(newComment);
			...
			widget: function( widgetHtml ) {
				$("table.Chatter-recent-comments").each(function() {
					var t = $(this);
					...
					Chatter.utility.tooltip($("tbody tr:first td a.tooltip", t));
					if (Chatter.utility.isElementVisible(newCommentsElement)) {
		...
		submit: function( trigger ) {
			...
			if ('' == commenttext) {
				$("label#commenttext_error").text(Chattertxt.required_field).show().focus();
				return false;
			}
			...
			// If Chatter thinks current user is logged in, check again for being logged in,
			// and check for a working connection to the server, to avoid hanging comments.
			if ( isUserLoggedIn && ( 'logged_in' != Chatter.utility.loggedInOut() ) ) {
				submitProgress.hide();
			...
			$.ajax( {
				...
				success: function( result ) {
					...
					$form.slideUp( 200, function() {
						...
						$( '#comment' ).val( '' );
						if ( errorMessage != "" )
							Chatter.utility.notification( errorMessage );

						Chatter.comment.fetch( false );
			...
			document.onkeypress = function(e) {
				...
				switch(keyCode) {
					...
					case 67:
						...
						if (!Chatter.utility.isElementVisible("#postbox"))
							$.scrollTo('#postbox', 50);
						$("#posttext").focus();
						if (e.preventDefault)
							e.preventDefault();
						...
						if (!commentLoop) {
							if (currPost > 0) {
								...
								if (0 != $('#'+postsOnPage[currPost]).children('ul.commentlist').length && !hidecomments) {
									...
									if (!Chatter.utility.isElementVisible('#'+commentsOnPost[currComment]))
										$.scrollTo('#'+commentsOnPost[currComment], 150);
									return;
								}
								if (!Chatter.utility.isElementVisible('#'+postsOnPage[currPost]))
					...
					case 79:
						...
						// "t" key
					case 84:
						Chatter.utility.jumpToTop();
						break;
					// "esc" key
					case 27:
						if (element.tagName == 'INPUT' || element.tagName == 'TEXTAREA') {
							if ( e.shiftKey && confirm( Chattertxt.comment_cancel_ays ) ) {
	...
	Chatter.plugins = {
		fluid: function( value ) {
			...
			// Sets the badge for Fluid app enclosed Chatter. See http://fluidapp.com/
	...
	Chatter.posts = {
		...
		edit: function( postId, element ) {
			...
			$.getJSON( ajaxUrl, { action: 'get_post', _inline_edit: nonce, post_ID: 'content-' + postId }, function( post ) {
				...
				if (post.post_format == 'standard' || post.post_type == 'page') {
					...
					defaultText(titleInput, Chattertxt.title);
				...
				Chatter.utility.autoGrowTextarea($(editor));
				...
				if (post.post_format == 'quote') {
					...
					defaultText(citationInput, Chattertxt.citation);
				...
				if ( post.post_type != 'page' ) {
					...
					defaultText(tagsInput, Chattertxt.tagit);
				...
				saveButton.innerHTML = Chattertxt.save;
				$( saveButton ).click( function() {
					var tags = ! tagsInput || tagsInput.value == Chattertxt.tagit ? '' : tagsInput.value;
					var args = {
						action:'save_post',
						_inline_edit: nonce,
					...
					if (post.post_format == 'standard' || post.post_type == 'page') {
						args.title = titleInput.value == Chattertxt.title ? '' : titleInput.value;
					} else if (post.post_format == 'quote') {
						args.citation = citationInput.value == Chattertxt.citation ? '' : citationInput.value;
					}
					...
					$.post(
						...
						function(result) {
							...
							$( document ).trigger( 'Chatter_edit_post_submit', { 'post_id' : postId, 'result' : result } );
						},
						'json');
				...
				cancelButton.innerHTML = Chattertxt.cancel;
				...
				// Initialize tags autocomplete
				if ( tagsInput )
					Chatter.autocomplete.tags( $( tagsInput ) );
			...
			ajaxCheckPosts = $.getJSON(queryString, function(newPosts){
				if (newPosts != null) {
					...
					if (!isFirstFrontPage || (typeof newPosts.html == "undefined") ) {
						...
						message = Chattertxt.n_new_updates.replace('%d', newUnseenUpdates) + " <a href=\"" + wpUrl +"\">"+Chattertxt.goto_homepage+"</a>";
						Chatter.utility.notification(message);
					} else {
						...
						newUpdatesLi.each(function() {
							...
							Chatter.utility.bindActions(this, 'post');
							if (Chatter.utility.isElementVisible(this) && !showNotification) {
								$(this).delay('250').animate({backgroundColor:'transparent'}, 2500, function() {
									$(this).removeClass('newupdates');
									Chatter.utility.titleCount();
								});
							}
							Chatter.utility.localizeMicroformatDates(this);
							counter++;
						});
						if (counter >= newPosts.numberofnewposts && showNotification) {
							var updatemsg = Chatter.utility.isElementVisible('#main > ul >li:first') ? "" :  "<a href=\"#\"  onclick=\"Chatter.utility.jumpToTop();\" \">"+Chattertxt.jump_to_top+"</a>" ;
							Chatter.utility.notification(Chattertxt.n_new_updates.replace('%d', counter) + " " + updatemsg);
							Chatter.utility.titleCount();
						}
					}
					$('.newupdates > h4, .newupdates > div').hover( Chatter.utility.removeYellow, Chatter.utility.removeYellow );
				}
			});
		...
		submit: function( trigger ) {
			...
			if ("" == posttext) {
				$("label#posttext_error").text(Chattertxt.required_field).show().focus();
				return false;
			}
			...
			$.ajax({
				...
				success: function(result) {
					if ("0" == result)
						errorMessage = Chattertxt.not_posted_error;

					$('#posttext').val('');
					$('#posttitle').val('');
					$('#postcitation').val('');
					$('#tags').val(Chattertxt.tagit);
					if(errorMessage != '')
						Chatter.utility.notification(errorMessage);
					...
					if (isFirstFrontPage && result != "0") {
						Chatter.posts.fetch(false);
					} else if (!isFirstFrontPage && result != "0") {
						Chatter.utility.notification(Chattertxt.update_posted);
					}
					...
					$( document ).trigger( 'Chatter_new_post_submit_success', { 'post_id' : result } );
				  },
				error: function(XMLHttpRequest, textStatus, errorThrown) {
					...
					$( document ).trigger( 'Chatter_new_post_submit_error', { 'request' : XMLHttpRequest, 'status' : textStatus, 'error' : errorThrown } );
				},
				timeout: 60000
			});
	...
	Chatter.utility = {
		autoGrowTextarea: function( textareas ) {
			...
			textareas.on('focus blur keydown paste', function(e) {
				...
				if ( 'paste' === e.type )
					setTimeout( function() { Chatter.utility.expandTextarea( t, $('#' + $(t).prop('id')).prop('scrollHeight') ); }, 30 );
				else
					Chatter.utility.expandTextarea( t, scrollHeight );
			});
		},
		bindActions: function( element, type ) {
			...
			switch (type) {
				case "comment" :
					...
					$(element).hover( Chatter.utility.removeYellow, Chatter.utility.removeYellow );
					if (inlineEditComments != 0 && isUserLoggedIn) {
						...
						thisCommentEditArea.editable(ajaxUrl, {event: 'edit', loadurl: ajaxUrl + '&action=get_comment&_inline_edit=' + nonce,
							...
							indicator : commentEditSpinnerText, loadtext: Chattertxt.loading, cancel: Chattertxt.cancel,
							submit  : Chattertxt.save, tooltip   : '', width: '90%', onblur: 'ignore',
				...
				case "post" :
					...
					if (inlineEditPosts != 0 && isUserLoggedIn) {
						...
						$( element ).find( 'a.edit-post-link:first' ).click(
							function(e) {
								var postId = this.rel;
								Chatter.posts.edit(postId, element);
								return false;
							});
					}
					$(".single #postlist li > div.postcontent, .single #postlist li > h4, li[id^='prologue'] > div.postcontent, li[id^='comment'] > div.commentcontent, li[id^='prologue'] > h4, li[id^='comment'] > h4").hover(function() {
		...
		localizeMicroformatDates: function( scopeElem ) {
			(scopeElem? $('abbr', scopeElem) : $('abbr')).each(function() {
				...
				if ( date && date > 0 )
					t.html(Chattertxt.date_time_format.replace('%1$s', locale.date(Chattertxt.time_format, date)).replace('%2$s', locale.date(Chattertxt.date_format, date)));
			});
		},
		loggedInOut: function() {
			 ...
			 *
			 * @uses ChatterAjax::logged_in_out()
		...
		removeYellow: function() {
			$('li.newcomment, tr.newcomment').each(function() {
				if (Chatter.utility.isElementVisible(this)) {
		...
		toggleUpdates: function( updater ) {
			switch (updater) {
				case "unewposts":
					if (0 == getPostsUpdate) {
						getPostsUpdate = setInterval(Chatter.posts.fetch, updateRate);
					}
					else {
				...
				case "unewcomments":
					if (0 == getCommentsUpdate) {
						getCommentsUpdate = setInterval(Chatter.comment.fetch, updateRate);
					}
					else {
	...
	if ((inlineEditPosts || inlineEditComments ) && isUserLoggedIn) {
		$.editable.addInputType('autogrow', {
			...
			plugin : function(settings, original) {
				window.Chatter.utility.autoGrowTextarea($('textarea', this));
			}
		});
	}
