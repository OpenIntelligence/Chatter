...
if ( ! class_exists( 'ChatterAjax_Read' ) ) {
	require dirname( __FILE__ ) . '/ajax-read.php';
}

if ( defined('DOING_AJAX') && DOING_AJAX && isset( $_REQUEST['Chatterajax'] ) ) {
	add_action( 'admin_init', array( 'ChatterAjax', 'dispatch' ) );
}

/*
 * We include all of ChatterAjax_Read's methods so that the old, deprecated API (db_version=1) still works for logged in users.
 ...
 * We only include in now for backward compatibility so that currently open Chatter windows continue to function.
 * By the time the next release rolls around, it'll be safe to remove.
 */
class ChatterAjax extends ChatterAjax_Read {
	static function dispatch() {
		$action = isset( $_REQUEST['action'] ) ? $_REQUEST['action'] : '';

		do_action( "Chatter_ajax", $action );
		if ( is_callable( array( 'ChatterAjax', $action ) ) )
			call_user_func( array( 'ChatterAjax', $action ) );
		else
			die( '-1' );
		exit;
	}
	...
	static function get_post() {
		...
		if ( !is_user_logged_in() ) {
			die( '<p>'.__( 'Error: not logged in.', 'Chatter' ).'</p>' );
		}
		$post_id = $_GET['post_ID'];
		$post_id = substr( $post_id, strpos( $post_id, '-' ) + 1 );
		if ( !current_user_can( 'edit_post', $post_id ) ) {
			die( '<p>'.__( 'Error: not allowed to edit post.', 'Chatter' ).'</p>' );
		}
		...
		$post_format = Chatter_get_post_format( $post_id );
	...
	static function get_comment() {
		...
		echo apply_filters( 'Chatter_get_comment_content', $comment->comment_content, $comment_id );
	}
	...
	static function save_post() {
		...
		if ( ! empty( $new_post_title ) )
			...
			$post_title = Chatter_title_from_content( $new_post_content );
		...
		if ( 'quote' == $post_format )
			$content = apply_filters( 'Chatter_get_quote_content', $post->post_content );
		else
		...
		echo json_encode( array(
			'title' => $post->post_title,
			'content' => $content,
			'tags' => get_tags_with_count( $post, '', __( '<br />Tags:' , 'Chatter' ) . ' ', ', ', ' &nbsp;' ),
		) );
	}
	...
	static function save_comment() {
		...
		if ( !current_user_can( 'edit_post', $comment->comment_post_ID ) ) {
			die( '<p>'.__( 'Error: not allowed to edit this comment.', 'Chatter' ).'</p>' );
		}
	...
	static function new_post() {
		...
		if ( ! ( current_user_can( 'publish_posts' ) ||
		        (get_option( 'Chatter_allow_users_publish' ) && $user_ID )) ) {

			die( '<p>'.__( 'Error: not allowed to post.', 'Chatter' ).'</p>' );
		}
		...
		if ( __( 'Tag it', 'Chatter' ) == $tags )
			$tags = '';

		// For empty or placeholder text, create a nice title based on content
		if ( empty( $title ) || __( 'Post Title', 'Chatter' ) == $title )
	    	$post_title = Chatter_title_from_content( $post_content );
		else
			$post_title = $title;

		$post_format = 'status';
		$accepted_post_formats = apply_filters( 'Chatter_accepted_post_cats', Chatter_get_supported_post_formats() ); // Keep 'Chatter_accepted_post_cats' filter for back compat (since Chatter 1.3.4)
