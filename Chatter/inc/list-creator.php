 ...
 * @package Chatter
 */
class Chatter_Post_List_Creator extends Chatter_List_Creator {
	var $form_action_name = 'Chatter-post-task-list';

	function Chatter_Post_List_Creator() {
		parent::Chatter_List_Creator();
	...
	function get_item_data( $task_id, $post_id = 0 ) {
		$meta = get_post_meta( $this->get_object_id( $post_id ), "Chatter_task_{$task_id}", true );
		if ( !$meta ) {
			return array();
		}
	...
	function put_item_data( $task_id, $done = true, $post_id = 0 ) {
		update_post_meta( $this->get_object_id( $post_id ), "Chatter_task_{$task_id}", sprintf( '%d:%d:%s', $done, get_current_user_id(), time() ) );
	}
	...
	function delete_item_data( $task_id, $post_id = 0 ) {
		delete_post_meta( $this->get_object_id( $post_id ), "Chatter_task_{$task_id}" );
	}
	...
	function get_all_item_data( $post_id = 0 ) {
		...
		$task_id_meta_keys = preg_grep( '/Chatter_task_\d/', $meta_keys );
 ...
 * @package Chatter
 */
class Chatter_Comment_List_Creator extends Chatter_List_Creator {
	var $form_action_name = 'Chatter-comment-task-list';

	function Chatter_Comment_List_Creator() {
		parent::Chatter_List_Creator();
		...
		// Renormalize task list meta into ASCII x's and o's
		add_filter( 'Chatter_get_comment_content', array( $this, 'unparse_comment_list' ), 10, 2 );
	...
	function get_item_data( $task_id, $comment_id = 0 ) {
		$meta = get_comment_meta( $this->get_object_id( $comment_id ), "Chatter_task_{$task_id}", true );
		if ( !$meta ) {
			return array();
		}
	...
	function put_item_data( $task_id, $done = true, $comment_id = 0 ) {
		update_comment_meta( $this->get_object_id( $comment_id ), "Chatter_task_{$task_id}", sprintf( '%d:%d:%s', $done, get_current_user_id(), time() ) );
	}
	...
	function delete_item_data( $task_id, $comment_id = 0 ) {
		delete_comment_meta( $this->get_object_id( $comment_id ), "Chatter_task_{$task_id}" );
	}
...
.Chatter-task-list ul {
	margin-left: 0 !important;
}
.Chatter-task-list ul ul {
	margin-left: 20px !important;
}
.Chatter-task-list li {
	list-style: none;
}
</style>
<?php
...
jQuery( function( $ ) {
	$( 'body' )
		.addClass( 'is-js' )
		.delegate( '.Chatter-task-list :checkbox', 'click', function() {
			var $this = $( this ),
...
<?php
	...
	function parse_list( $text, $doing_recursion = false ) {
		...
		// Run our regex through the callback, get the eventual text a few levels down and return it back to Chatter here.
	...
	function _do_list_callback( $matches ) {
		...
		case 'o' : // Task List
			...
			// Add form
			$ajax_url = remove_query_arg( 'Chatterajax', Chatter_JS::ajax_url() );
			$return  = sprintf( '<form class="Chatter-task-list" id="Chatter-task-list-%d" action="%s" method="post">', $id, esc_url( $ajax_url ) ) . $return;
			...
			$return .= wp_nonce_field( "Chatter-task-list_$id", "_Chatter_task_list_nonce_$id", true, false );
			$return .= "\n</p>\n</form>";
	...
	function process_task_list_items( $text, $indent, $context = 'display' ) {
		...
		if ( 'edit' == $context ) {
			...
			$format = '<li id="Chatter-task-%1$d-%2$d"><label>%8$s<input type="checkbox" name="Chatter_task[%1$d][%2$d]"%3$s%4$s value="1" /><input type="hidden" name="Chatter_task_ids[%1$d][%2$d]" value="%6$s" /> %5$s%10$s%8$s</label></li>';
		}
	...
	function submit() {
		...
		if ( $is_ajax ) {
			check_ajax_referer( "Chatter-task-list_$id", "_Chatter_task_list_nonce_$id" );
		} else {
			check_admin_referer( "Chatter-task-list_$id", "_Chatter_task_list_nonce_$id" );
		}

		foreach ( $_POST['Chatter_task_ids'] as $object_id => $tasks ) {
			foreach ( $tasks as $task_id => $checked_in_post_state ) {
				$checked_now = isset( $_POST['Chatter_task'][$object_id][$task_id] ) && $_POST['Chatter_task'][$object_id][$task_id];
				$checked_in_post = 'x' == $checked_in_post_state;
